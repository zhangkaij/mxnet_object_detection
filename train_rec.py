...
def validate(net, val_data, ctx, eval_metric):
    ...
    for batch in val_data:
        ...
        for x, y, im_scale in zip(*batch):
            ...
            gt_difficults.append(y.slice_axis(axis=-1, begin=5, end=6) if y.shape[-1] > 5 else np.zeros(gt_ids[-1].shape))

        # update metric
        for det_bbox, det_id, det_score, gt_bbox, gt_id, gt_diff in zip(det_bboxes, det_ids, det_scores, gt_bboxes, gt_ids, gt_difficults):
            #print(det_bbox.shape, det_id.shape, det_score.shape, gt_bbox.shape, gt_id.shape)
...
def train(net, train_data, val_data, eval_metric, ctx, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(train_data):
            ...
            #if keep_init == False:
            #    keep_init = True
            #    #add by kaijun.zhang 20181102
            #    net.save_parameters("faster_rcnn_init.params")
...
if __name__ == '__main__':
    ...
    net = get_model(net_name, pretrained=True, root='./models')
    net.reset_class(rec_classes)
